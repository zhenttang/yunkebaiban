...
export const CloudStorageProvider = ({ children, serverUrl = getSocketIOUrl() // 使用内联配置管理
 }) => {
    ...
    // 保存离线操作 - 按照AFFiNE标准格式
    const saveOfflineOperation = async (docId, update) => {
    ...
    // 推送文档更新 - 增强版本支持队列
    const pushDocUpdate = async (docId, update) => {
        ...
        try {
            ...
            const requestData = {
                spaceType: 'workspace', // 按照AFFiNE标准：spaceType
    ...
    useEffect(() => {
        window.__CLOUD_STORAGE_MANAGER__ = value;
        return () => {
            delete window.__CLOUD_STORAGE_MANAGER__;
        };
    }, [value]);
    return (_jsx(CloudStorageContext.Provider, { value: value, children: children }));
};
//# sourceMappingURL=cloud-storage-manager.js.map