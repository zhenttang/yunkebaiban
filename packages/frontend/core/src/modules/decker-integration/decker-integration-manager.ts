...
export class DeckerIntegrationManager {
  private store: BlockSuiteStore | null = null;
  ...
  setStore(store: BlockSuiteStore) {
    this.store = store;
  }
    ...
    const globalHandler = (event: MessageEvent) => {
      ...
      switch (type) {
        case 'DECK_EXPORT_COMPLETE':
          this.handleDeckerExport(event.data as DeckerExportData);
          break;
        case 'DECK_READY':
  ...
  private async handleDeckerExport(data: DeckerExportData) {
    if (!this.store) {
  ...
  private async createImageBlock(sourceId: string, customData: string, size: number) {
  ...
  // 打开Decker编辑器进行编辑
  openDeckerForEdit(deckData: string) {
    this.openDeckerModal(deckData);
  }

  private openDeckerModal(deckData?: string) {
    // 移除可能存在的旧模态框
...
export function openDeckerEditorWithData(deckData: string) {
