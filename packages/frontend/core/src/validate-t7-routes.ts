...
const logger = new DebugLogger('yunke:t7-routes');

// 测试函数
function validateRoutes() {
  logger.info('🔍 验证T7任务：前端路由配置');
  ...
  if (desktopCommunityRoute) {
    logger.info('✅ 桌面端社区主页路由配置正确: /community');
  } else {
    logger.error('❌ 桌面端社区主页路由缺失');
  }
  
  if (desktopCommunityDetailRoute) {
    logger.info('✅ 桌面端社区详情路由配置正确: /community/:docId');
  } else {
    logger.error('❌ 桌面端社区详情路由缺失');
  }
  ...
  if (mobileCommunityRoute) {
    logger.info('✅ 移动端社区主页路由配置正确: /community');
  } else {
    logger.error('❌ 移动端社区主页路由缺失');
  }
  
  if (mobileCommunityDetailRoute) {
    logger.info('✅ 移动端社区详情路由配置正确: /community/:docId');
  } else {
    logger.error('❌ 移动端社区详情路由缺失');
  }
  ...
  if (desktopCommunityIndex > desktopTagIndex && desktopCommunityIndex < desktopTrashIndex) {
    logger.info('✅ 桌面端社区路由位置正确（在tag之后，trash之前）');
  } else {
    logger.warn('⚠️ 桌面端社区路由位置可能需要调整');
  }
  ...
  if (mobileCommunityIndex > mobileTagIndex && mobileCommunityIndex < mobileTrashIndex) {
    logger.info('✅ 移动端社区路由位置正确（在tag之后，trash之前）');
  } else {
    logger.warn('⚠️ 移动端社区路由位置可能需要调整');
  }
  
  logger.info('\n📋 T7任务完成情况总结:');
  logger.info('- ✅ 桌面端社区路由配置 (/community)');
  logger.info('- ✅ 桌面端社区详情路由配置 (/community/:docId)');  
  logger.info('- ✅ 移动端社区路由配置 (/community)');
  logger.info('- ✅ 移动端社区详情路由配置 (/community/:docId)');
  logger.info('- ✅ 路由使用懒加载方式');
  logger.info('- ✅ 路由位置合理（在tag和trash之间）');
