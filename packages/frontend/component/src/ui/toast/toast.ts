...
const htmlToElement = <T extends ChildNode>(html: string | TemplateResult) => {
  ...
  if (typeof html === 'string') {
    html = html.trim(); // Never return a whitespace text node as a result
...
export const toast = (
  ...
  if (ToastContainer && ToastContainer.children.length >= 2) {
    // If there are already two toast messages, immediately remove the oldest one
    const oldestToast = ToastContainer.children[0] as HTMLDivElement;
    oldestToast.remove();
  }

  // If there is already one toast message, start its fade out animation
