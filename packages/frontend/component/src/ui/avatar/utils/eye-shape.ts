...
export function generateEyePoints(rands: EyeParameters, width: number = 50): EyePoints {
  ...
  const upper_eyelid_points: number[][] = [];
  const upper_eyelid_points_left_control: number[][] = [];
  const upper_eyelid_points_right_control: number[][] = [];
  ...
  const lower_eyelid_points: number[][] = [];
  const lower_eyelid_points_left_control: number[][] = [];
  const lower_eyelid_points_right_control: number[][] = [];
  ...
  const eyeCenter = [upper_eyelid_points[50][0] / 2.0 + lower_eyelid_points[50][0] / 2.0, upper_eyelid_points[50][1] / 2.0 + lower_eyelid_points[50][1] / 2.0];
  ...
  return { upper: upper_eyelid_points, lower: lower_eyelid_points, center: [[0, 0]] };
}
...
export function generateBothEyes(width: number = 50): BothEyes {
  ...
  for (const key in rands_right) {
    ...
    if (typeof rands_right[key as keyof EyeParameters] === 'number') {
      ...
      (rands_right[key as keyof EyeParameters] as number) += randomFromInterval(-(rands_right[key as keyof EyeParameters] as number) / 2.0, (rands_right[key as keyof EyeParameters] as number) / 2.0);
    }
  }
  ...
  for (const key in left_eye) {
    if (typeof left_eye[key as keyof EyePoints] === 'object') {
      const points = left_eye[key as keyof EyePoints] as number[][];
