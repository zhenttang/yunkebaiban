...
export class CloudDocStorage extends DocStorageBase<CloudDocStorageOptions> {
  ...
  get socket() {
    // 🔧 如果连接断开，记录警告（实际重连逻辑在 pushDocUpdate 中处理）
    if (!this.connection.inner.socket?.connected) {
      console.warn('⚠️ [CloudDocStorage.socket] Socket 未连接', {
  ...
  override async pushDocUpdate(update: DocUpdate) {
    ...
    // 🔧 修复：如果连接断开，尝试重新连接
    if (!this.connection.inner.socket?.connected) {
      console.warn('⚠️ [CloudDocStorage.pushDocUpdate] Socket 未连接，尝试重新连接...', {
        docId: update.docId,
        connectionStatus: this.connection.status
      });

      // 如果连接处于 error 或 closed 状态，主动触发连接
      if (this.connection.status === 'error' || this.connection.status === 'closed' || this.connection.status === 'idle') {
        console.log('🔄 [CloudDocStorage.pushDocUpdate] 主动触发重连');
      ...
      try {
        await this.connection.waitForConnected(timeout);
        console.log('✅ [CloudDocStorage.pushDocUpdate] 重连成功');
      } catch (err) {
        console.error('❌ [CloudDocStorage.pushDocUpdate] 重连超时', err);
