...
export const themeToVar = (themeName: keyof YunkeTheme) =>
  `--yunke-${camelToKebab(themeName)}` as keyof YunkeCssVariables;
...
export type YunkeTheme = typeof lightTheme & {
  editorMode: 'page' | 'edgeless';
};

export type YunkeCssVariables = {
  [Key in `--yunke-${Kebab<keyof YunkeTheme>}`]: string;
};
...
/**
 * Get Yunke css variable name type safely
...
export function cssVar(
  ...
  const varName = key.startsWith('--')
    ? key
    : themeToVar(key as keyof YunkeTheme);
