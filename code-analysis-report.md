...
**修复状态** (2025-11-13):
- ✅ 帖子详情页引入 `loadPostData`，加载失败时会展示重试按钮、弹出 toast，并记录错误内容，避免静默失败。
- ✅ 加入组件卸载守卫，防止 Promise 完成后对卸载组件 setState 造成的内存泄漏警告。
...
**修复状态** (2025-11-13):
- ✅ `page-detail-editor` 使用 `useRef` 实时保存 `mode`，事件处理器读取最新值，不再依赖旧闭包；同时 effect 仍只绑定一次监听，避免重复注册。
