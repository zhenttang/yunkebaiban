...
class PasteTr {
  ...
  focusPasted = () => {
    ...
    const cursorBlock =
      this.pointState.model.flavour === 'yunke:code' || !this.lastSnapshot
  ...
  constructor(
    ...
    this.firstSnapshotIsPlainText =
      this.firstSnapshot.flavour === 'yunke:paragraph' &&
  ...
  merge() {
    ...
    if (this.pointState.model.flavour === 'yunke:code') {
      this._mergeCode();
      return;
    }
...
function flatNote(snapshot: SliceSnapshot) {
  if (snapshot.content[0]?.flavour === 'yunke:note') {
