  ...
  ({ slots, docCRUD, assetsManager }) => {
    ...
    const replaceSurfaceRefIdSubscription = afterImportBlock$
      ...
      .subscribe(model => {
        ...
        // Otherwise,
        // 1. If the reference is an yunke:frame not in doc, generate a new id.
        // 2. If the reference is graph, keep the original id.
        if (idMap.has(original)) {
          model.props.reference = idMap.get(original)!;
        } else if (
          model.props.refFlavour === 'yunke:frame' &&
    ...
    const beforeImportBlockSubscription = slots.beforeImport
      ...
      .subscribe(payload => {
        ...
        if (snapshot.flavour === 'yunke:page') {
          const index = snapshot.children.findIndex(
            c => c.flavour === 'yunke:surface'
          );
          if (index !== -1) {
        ...
        // Should be re-paired.
        if (['yunke:attachment', 'yunke:image'].includes(snapshot.flavour)) {
        ...
        if (snapshot.flavour === 'yunke:surface') {
