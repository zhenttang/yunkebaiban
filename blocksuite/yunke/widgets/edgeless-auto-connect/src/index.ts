...
import { SmallDocIcon } from '@blocksuite/yunke-components/icons';
import {
  FrameBlockModel,
  NoteBlockModel,
  NoteDisplayMode,
...
import { matchModels, stopPropagation } from '@blocksuite/yunke-shared/utils';
...
export class EdgelessAutoConnectWidget extends WidgetComponent<RootBlockModel> {
  ...
  private _initLabels() {
    ...
    const surfaceRefs = this.block.std.store
      .getBlocksByFlavour('yunke:surface-ref')
    ...
    this._disposables.add(
      this.store.slots.blockUpdated.subscribe(payload => {
        if (payload.flavour === 'yunke:surface-ref') {
          switch (payload.type) {
            case 'add':
