import { DatabaseSelection } from '@blocksuite/yunke-block-database';
import { EdgelessLegacySlotIdentifier } from '@blocksuite/yunke-block-surface';
import { TableSelection } from '@blocksuite/yunke-block-table';
import {
  darkToolbarStyles,
...
export class YunkeToolbarWidget extends WidgetComponent {
  ...
  updateWithSurface(
    ...
    if (activated && surface) {
      ...
      if (hasLocked) {
        flavour = 'yunke:surface:locked';
      } else {
        if (paired.length === 1) {
          flavour = paired[0][0];
          if (flavour === 'yunke:surface:shape' && paired[0][1].length === 1) {
      ...
      if (!sideOptions) {
        ...
        if (flavours.has('yunke:surface:frame')) {
          sideOptions = sideMap.get('yunke:surface:frame') ?? null;
        } else if (flavours.has('yunke:surface:group')) {
          sideOptions = sideMap.get('yunke:surface:group') ?? null;
        }
      }
    }
    ...
    disposables.add(
      std.selection.find$(TextSelection).subscribe(result => {
        ...
        batch(() => {
          ...
          flavour$.value = 'yunke:note';
          placement$.value = toolbarRegistry.getModulePlacement('yunke:note');
      ...
      std.selection.filter$(BlockSelection).subscribe(selections => {
        ...
        batch(() => {
          ...
          placement$.value = toolbarRegistry.getModulePlacement(
            flavour,
            flavour === 'yunke:note' ? 'top' : 'top-start'
          );
