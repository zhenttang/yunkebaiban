...
export const sideMap = new Map([
  ...
  ['yunke:surface:frame', { top: 28 }],
  // includes group element
  ['yunke:surface:group', { top: 20 }],
  // has only one shape element
  ['yunke:surface:shape', { top: 26, bottom: -26 }],
]);
...
export function autoUpdatePosition(
  ...
  const isInline = flavour === 'yunke:note';
...
export function renderToolbar(
  ...
  const actions = [
    flavour,
    `custom:${flavour}`,
    hasSurfaceScope ? ['yunke:surface:*', 'custom:yunke:surface:*'] : [],
    'yunke:*',
    'custom:yunke:*',
  ]
    .flat()
