...
import { FontFamilyIcon } from '@blocksuite/yunke-components/icons';
import {
  mountShapeTextEditor,
  SHAPE_OVERLAY_HEIGHT,
  SHAPE_OVERLAY_WIDTH,
  ShapeComponentConfig,
} from '@blocksuite/yunke-gfx-shape';
import {
  insertEdgelessTextCommand,
  mountTextElementEditor,
} from '@blocksuite/yunke-gfx-text';
import type {
  Connection,
...
export class EdgelessAutoCompletePanel extends WithDisposable(LitElement) {
  ...
  private _addFrame() {
    ...
    const props = this.std.get(EditPropsStore).applyLastProps('yunke:frame', {
      title: new Y.Text(`Frame ${frameIndex}`),
      xywh: serializeXYWH(...xywh),
      presentationIndex: frameMgr.generatePresentationIndex(),
    });
    const id = this.crud.addBlock('yunke:frame', props, surfaceBlockModel);
  ...
  private _addNote() {
    ...
    store.addBlock('yunke:paragraph', { type: 'text' }, id);
  ...
  private _showNoteOverlay() {
    ...
    const background = this.edgeless.std
      .get(ThemeProvider)
      .getColorValue(
        this.edgeless.std.get(EditPropsStore).lastProps$.value['yunke:note']
          .background,
  ...
  override render() {
    ...
    return html`<div class="auto-complete-panel-container" style=${style}>
      ...
      >
        <div class="row-button">添加相同对象</div>
      </edgeless-tool-icon-button>
    </div>`;
  }
