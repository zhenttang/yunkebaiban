...
import { ParagraphBlockComponent } from '@blocksuite/yunke-block-paragraph';
import { DropIndicator } from '@blocksuite/yunke-components/drop-indicator';
import {
...
import { surfaceRefToEmbed } from '@blocksuite/yunke-shared/adapters';
import {
...
export class DragEventWatcher {
    ...
    if (edge === 'right' && matchModels(dropModel, [ListBlockModel])) {
      ...
      if (
        snapshot.content.every(block =>
          schema.safeValidate(block.flavour, 'yunke:list')
        )
      ) {
      ...
      const placement =
        isDropOnNoteBlock &&
        schema.safeValidate(snapshot.content[0].flavour, 'yunke:note')
          ? 'in'
          : edge === 'top'
            ? 'before'
            : 'after';
  ...
  private readonly _onPageDrop = (
    ...
    if (
      ...
      snapshot.content.every(block => block.flavour === 'yunke:note')
    ) {
    ...
    if (
      ...
      if (dragPayload.from?.docId === this.widget.store.id) {
        ...
        const walk = (block: BlockSnapshot) => {
          if (block.flavour === 'yunke:surface') {
            Object.values(
  ...
  private readonly _rewriteSnapshotXYWH = (
    ...
    const rewrite = (block: BlockSnapshot) => {
      if (block.flavour === 'yunke:surface') {
        ...
        if (block.flavour === 'yunke:embed-iframe') {
          ...
          block.flavour === 'yunke:attachment' ||
          block.flavour === 'yunke:bookmark' ||
          block.flavour.startsWith('yunke:embed-')
        ) {
  ...
  private readonly _dropAsGfxBlock = (
    ...
    const groupByParent = groupBy(snapshot.content, block =>
      schema.safeValidate(block.flavour, 'yunke:surface')
        ? 'yunke:surface'
        : schema.safeValidate(block.flavour, 'yunke:page')
          ? 'yunke:page'
    ...
    if (!groupByParent.empty?.length) {
      ...
      if (groupByParent['yunke:surface']) {
        const content = groupByParent['yunke:surface'];
      ...
      if (groupByParent['yunke:page']) {
        const content = groupByParent['yunke:page'];
        const pageSnapshot = {
          ...snapshot,
          content,
        };
      ...
      const content = snapshot.content.filter(block =>
        schema.safeValidate(block.flavour, 'yunke:note')
      );
  ...
  private async _dropToModel(
    ...
    try {
      ...
      if (snapshot.content.length === 1) {
        const [first] = snapshot.content;
        if (first.flavour === 'yunke:embed-linked-doc') {
  ...
  private _makeDropTarget(view: BlockComponent) {
    ...
    if (
      // yunke:surface block can't be drop target in any modes
