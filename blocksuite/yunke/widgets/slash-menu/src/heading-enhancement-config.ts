  ...
  items: (ctx: SlashMenuContext) => [
    {
      ...
      when: (ctx) => {
        // 只在段落块中显示
        return ctx.model.flavour === 'yunke:paragraph';
      },
      action: async (ctx) => {
        const { std, model } = ctx;
      ...
      action: async (ctx) => {
        ...
        try {
          ...
          const countBlocks = (block: any) => {
            if (block.flavour === 'yunke:paragraph' && block.props.text) {
      ...
      action: async (ctx) => {
        ...
        try {
          ...
          const analyzeBlocks = (block: any) => {
            if (block.flavour === 'yunke:paragraph' && block.props.type?.startsWith('h')) {
              headings.push({
                id: block.id,
