import { autoResizeElementsCommand } from '@blocksuite/yunke-block-surface';
...
const DEFAULT_ATTACHMENT_NAME = 'yunke-attachment';
...
export async function turnImageIntoCardView(
  ...
  if (!doc.schema.flavourSchemaMap.has('yunke:attachment')) {
    console.error('不支持附件类型！');
    return;
  }
  ...
  transformModel(model, 'yunke:attachment', attachmentProp);
}
...
function hasExceeded(
  ...
  if (exceeded) {
    ...
    toast(std.host, `您只能上传小于 ${size} 的文件`);
  }
...
async function buildPropsWith(std: BlockStdScope, file: File) {
  ...
  if (!(imageSize.width * imageSize.height)) {
    toast(std.host, '无法读取图片尺寸，请尝试其他图片');
