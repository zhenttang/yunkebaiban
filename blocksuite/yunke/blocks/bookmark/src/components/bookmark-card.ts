import { getEmbedCardIcons } from '@blocksuite/yunke-block-embed';
import { LoadingIcon, WebIcon16 } from '@blocksuite/yunke-components/icons';
import { ImageProxyService } from '@blocksuite/yunke-shared/adapters';
import { ThemeProvider } from '@blocksuite/yunke-shared/services';
import { getHostName } from '@blocksuite/yunke-shared/utils';
...
export class BookmarkCard extends SignalWatcher(
  ...
  override render() {
    ...
    const titleIcon = this.loading
      ? LoadingIcon()
      : icon
        ? html`<img src=${imageProxyService.buildUrl(icon)} alt="icon" crossorigin="anonymous" />`
        : WebIcon16;
    ...
    const bannerImage =
      !this.loading && image
        ? html`<img src=${imageProxyService.buildUrl(image)} alt="banner" crossorigin="anonymous" />`
        : EmbedCardBannerIcon;
