...
> = (ctx, next) => {
  ...
  const appendDivider: Command<{}, { updatedBlocks: BlockModel[] }> = (
    ...
    const id = doc.addBlock('yunke:divider', {}, parent, index + 1);
    if (!nextSibling) {
      nextSiblingId = doc.addBlock('yunke:paragraph', {}, parent);
    }
  ...
  const [result, resultCtx] = std.command
    ...
    .try(chain => [
      chain.pipe((_, next) => {
        if (['yunke:code', 'yunke:divider'].includes(flavour)) {
          return next();
        }
        return false;
      }),
