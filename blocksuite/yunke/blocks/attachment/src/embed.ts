...
import { FileSizeLimitProvider } from '@blocksuite/yunke-shared/services';
import {
  readImageSize,
  transformModel,
  withTempBlobData,
...
async function turnIntoImageBlock(model: AttachmentBlockModel) {
  if (!model.store.schema.flavourSchemaMap.has('yunke:image')) {
  ...
  transformModel(model, 'yunke:image', imageProp);
}
