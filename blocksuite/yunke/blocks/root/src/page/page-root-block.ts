import { appendParagraphCommand } from '@blocksuite/yunke-block-paragraph';
import {
  CodeBlockModel,
  ListBlockModel,
  NoteBlockModel,
...
export class PageRootBlockComponent extends BlockComponent<RootBlockModel> {
  ...
  private _createDefaultNoteBlock() {
    ...
    const noteId = store.addBlock('yunke:note', {}, store.root?.id);
  ...
  private _getDefaultNoteBlock() {
    return (
      this.store.root?.children.find(
        child => child.flavour === 'yunke:note'
  ...
  override connectedCallback() {
    ...
    this.bindHotKey({
      ...
      ArrowUp: () => {
        ...
        if (!prevNote || prevNote.flavour !== 'yunke:note') {
        ...
        const notes = this.store.getModelsByFlavour('yunke:note');
