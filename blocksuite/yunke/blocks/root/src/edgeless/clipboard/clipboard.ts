import { addAttachments } from '@blocksuite/yunke-block-attachment';
import { EdgelessFrameManagerIdentifier } from '@blocksuite/yunke-block-frame';
import { addImages } from '@blocksuite/yunke-block-image';
import {
  CanvasRenderer,
  DefaultTool,
...
import { splitIntoLines } from '@blocksuite/yunke-gfx-text';
import type {
  EmbedCardStyle,
  ShapeElementModel,
  ...
  private async _pasteTextContentAsNote(content: BlockSnapshot[] | string) {
    ...
    if (typeof content === 'string') {
      ...
      for (let index = 0; index < content.length; index++) {
        ...
        if (blockSnapshot.flavour === 'yunke:note') {
