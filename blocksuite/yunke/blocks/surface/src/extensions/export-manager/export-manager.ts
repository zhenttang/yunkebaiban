import { ImageBlockModel, type RootBlockModel } from '@blocksuite/yunke-model';
import { FetchUtils } from '@blocksuite/yunke-shared/adapters';
import {
  CANVAS_EXPORT_IGNORE_TAGS,
  DEFAULT_IMAGE_PROXY_ENDPOINT,
} from '@blocksuite/yunke-shared/consts';
import type { Viewport } from '@blocksuite/yunke-shared/types';
import {
  isInsidePageEditor,
  matchModels,
...
export class ExportManager {
  ...
  async edgelessToCanvas(
    ...
    if (!(surfaceRenderer instanceof CanvasRenderer)) {
      console.warn(
        'ExportManager.edgelessToCanvas使用了无效的渲染器类型。期望CanvasRenderer。'
      );
      return undefined;
    }
