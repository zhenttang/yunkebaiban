import { ParagraphBlockModel } from '@blocksuite/yunke-model';
...
export function toCode(
  ...
  const codeId = doc.addBlock('yunke:code', { language }, parent, index);

  if (model.text && model.text.length > prefixText.length) {
    const text = model.text.clone();
    doc.addBlock('yunke:paragraph', { text }, parent, index + 1);
