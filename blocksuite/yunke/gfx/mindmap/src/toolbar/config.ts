import { hasGrouped } from '@blocksuite/yunke-gfx-shape';
import {
  LayoutType,
  MindmapElementModel,
  MindmapStyle,
...
import { getMostCommonValue } from '@blocksuite/yunke-shared/utils';
import {
  type MenuItem,
  renderMenu,
...
export const mindmapToolbarExtension = ToolbarModuleExtension({
  id: BlockFlavourIdentifier('yunke:surface:mindmap'),
  config: mindmapToolbarConfig,
});

export const shapeMindmapToolbarExtension = ToolbarModuleExtension({
  id: BlockFlavourIdentifier('custom:yunke:surface:shape'),
