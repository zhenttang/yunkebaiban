 ...
 *
 * To add Turbo Rendering support for a new block type (e.g., 'yunke:my-block'):
 *
 * 1.  **In the block's package (e.g., `blocksuite/yunke/blocks/my-block`):**
 *   a.  Add `@blocksuite/yunke/gfx/turbo-renderer` as a dependency in `package.json` and create a `src/turbo` directory.
 *   b.  Implement the Layout Handler (e.g., `MyBlockLayoutHandlerExtension`) and Painter Worker (e.g., `MyBlockLayoutPainterExtension`). Refer to `ParagraphLayoutHandlerExtension` and `ParagraphLayoutPainterExtension` in `blocksuite/yunke/blocks/block-paragraph` for implementation examples.
 ...
 *   e.  Add a TypeScript project reference to `blocksuite/yunke/gfx/turbo-renderer` in `tsconfig.json`.
 *
 ...
 *
 * 3.  **Run `yarn yunke init`** from the workspace root to update generated configuration files (`workspace.gen.ts`) and the lockfile (`yarn.lock`).
 *
 * **Note:** Always ensure the directory structure and export patterns match the `paragraph` block (`blocksuite/yunke/blocks/block-paragraph`) for consistency.
 */
