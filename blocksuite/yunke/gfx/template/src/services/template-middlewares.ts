import { generateElementId, sortIndex } from '@blocksuite/yunke-block-surface';
import type { ConnectorElementModel } from '@blocksuite/yunke-model';
...
export const replaceIdMiddleware = (job: TemplateJob) => {
  ...
  const regenerateBlockId = (data: SlotBlockPayload['data']) => {
    ...
    if (blockJson.flavour === 'yunke:surface-ref') {
      assertType<
        SnapshotNode<{
          reference: string;
        }>
    ...
    if (blockJson.flavour === 'yunke:surface') {
    ...
    if (blockJson.flavour === 'yunke:frame') {
...
export const createStickerMiddleware = (
  ...
  return (job: TemplateJob) => {
    ...
    const changeInserPosition = (blockJson: BlockSnapshot) => {
      if (blockJson.flavour === 'yunke:image' && blockJson.props.xywh) {
        const bound = Bound.deserialize(blockJson.props['xywh'] as string);
