...
// 响应式功能集成
import { ResponsiveManager, DEFAULT_BREAKPOINTS } from '@blocksuite/yunke-layout-interactions/responsive/responsive-manager';
...
export class LayoutSwitcher extends LitElement implements ILayoutSwitcher {
  ...
  private async setupLayoutService(): Promise<void> {
    try {
      // 动态导入Mock服务
      const { MockPageLayoutService } = await import('@blocksuite/yunke-layout-testing/mocks/core-services');
