...
export class ContentAwareDistributor implements DistributionAlgorithm {
  ...
  private _getContentScore(block: Block): number {
    // 为不同类型的内容分配权重
    switch (block.flavour) {
      case 'yunke:heading': return 10;
      case 'yunke:image': return 8;
      case 'yunke:database': return 7;
      case 'yunke:code': return 6;
      case 'yunke:callout': return 5;
      case 'yunke:list': return 4;
      case 'yunke:paragraph': return 2;
      default: return 1;
    }
  }
  ...
  private _calculateBlockSimilarity(block1: Block, block2: Block): number {
    ...
    // 检查语义相关性
    const textTypes = ['yunke:paragraph', 'yunke:heading', 'yunke:list'];
    const mediaTypes = ['yunke:image', 'yunke:video'];
