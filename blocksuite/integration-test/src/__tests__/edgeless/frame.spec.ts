...
import type { YunkeFrameTitleWidget } from '@blocksuite/yunke/widgets/frame-title';
  ...
  test('frame should have title', async () => {
    ...
    const getFrameTitle = (frameId: string) => {
      const frameTitleWidget = service.std.view.getWidget(
        'yunke-frame-title-widget',
        frameId
      ) as YunkeFrameTitleWidget | null;
      return frameTitleWidget?.shadowRoot?.querySelector('yunke-frame-title');
    };
